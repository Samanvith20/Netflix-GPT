...
const MovieCard =({poster, title}) => {
    ...
    return (
        <div className="xl:w-[170px] lg:w-[150px] md:w-[130px] sm:w-[120px] w-[90px] cursor-pointer hover:opacity-90 hover:scale-105">
            <img className="w-full stroke animate  rounded-lg" src={IMG_CDN_URL + poster} alt="Loading.."></img>
            <span className="text-gray-300 hidden md:block lg:text-base md:text-sm  ml-3">{title?.length > 18 ? `${title.slice(0, 18)}...` : title} </span>
